...
**Shipped (15/16 plan tasks; Task 15 partial — review done, DEFERRED struck, merge awaits founder approval after live E2E):**

| Task | Commit                | What                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 0    | —                     | Worktree scaffold + `.env.local` copy + baseline.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 1    | `292c544`             | Schema: `consultations` + `questionnaire_responses` + `photos` + `consultation_status_history` tables. `convex/lib/photoSlot.ts` (PHOTO_SLOTS, REQUIRED_PHOTO_COUNT=4). `convex/consultations/schema.ts` (22-status validator, 5-vertical, 3-mime, 4-kind).                                                                                                                                                                                                                                                                                                                                                                                                 |
| 2    | `889cf46` + `4b12a84` | Transition engine — `transitionStatus` + `systemTransition` internalMutations + audit history. 7 tests.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 3    | `5af8b37`             | 28-Q hair-loss bank verbatim port from `docs/VERTICAL-HAIR-LOSS.md §4`. Extended `Question` type (number/freetext, sections, sexGate, skipIf, maleOptions/femaleOptions, femaleOnly/maleOnly). Compat patch on `ed.ts` + renderer + 4 existing tests. 10-assertion shape test. APPROVED_WITH_MINORS — important findings #1–#3 (renderer doesn't honor new fields) explicitly resolved by Tasks 4 + 5.                                                                                                                                                                                                                                                      |
| 4    | `787f5fd`             | Pure `skipLogic.ts` — `getReachableQids` + `getNextQid` + `AnswersMap`. 7 Jest tests. APPROVED clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 5    | `c4fa297`             | Store: added `schemaVersion`/`currentQid`/`history` + `startHL`/`advance`/`goBack`. Router `[qid].tsx`: replaced `index+1` with `getNextQid`; `resolveOptions` helper for Q3 male/female + femaleOnly/maleOnly filter; new `number` type branch with min/max validation; variant (b) inline-newAnswers ordering avoids race. `[condition]/index.tsx` calls `startHL` for hair-loss. 3 store tests. APPROVED clean.                                                                                                                                                                                                                                          |
| 6    | `3428510`             | Review screen rewrite — section grouping (6 HL sections via SECTION_TITLES); `getReachableQids` filter (empty sexual_health header dropped for female); sex-aware `labelForAnswer` resolves Q3 male/female labels; tappable rows → `/questionnaire/${condition}/${qid}` for edit; consent checkbox (accessibilityRole="checkbox") gates Submit; `colors.accentWarm` per CLAUDE.md CTA rule; TODO names Task 14 above placeholder submit. 4 Jest tests (replaced prior 2). APPROVED_WITH_MINORS — single nit `color:"white"` literal on review.tsx:211, fold into Task 15 cleanup.                                                                           |
| 7    | `5c52b22`             | Expo deps + config — `expo-camera@17.0.10`, `expo-image-picker@17.0.10`, `expo-apple-authentication@8.0.8` exact-pinned in `apps/mobile/package.json`. `app.config.ts` (NOT `app.json` per repo convention) plugins array extended with all 3 plugin entries + camera/photos permission strings. `npx expo prebuild --no-install --platform android` dry-run passed; generated `android/`+`ios/` removed (gitignored). No jest mocks needed yet (no production imports).                                                                                                                                                                                    |
| 8    | `60b173e`             | Photo-slot bottom sheet — `apps/mobile/src/components/questionnaire/PhotoSlotBottomSheet.tsx` (66 lines) reuses existing `@/components/ui/BottomSheet` primitive (don't hand-roll). `SLOT_LABELS` lookup. `apps/mobile/src/types/photo-slot.ts` shim mirrors convex `PhotoSlot` union. 2 Jest tests.                                                                                                                                                                                                                                                                                                                                                        |
| 9    | `488955c`             | Real camera + library picker — `app/photo-upload/camera.tsx` rewritten with `CameraView` + `useCameraPermissions` (permission-denied screen, capture-then-preview, Retake/Use this photo). `[condition].tsx` switched to canonical `PHOTO_SLOTS` (crown/hairline/left_temple/right_temple) — legacy human labels dropped; weight vertical out of scope. `pickFromLibrary.ts` helper returns boolean. `jest.setup.ts` mocks for `expo-camera` + `expo-image-picker`. 2 existing test files rewritten for new flow. APPROVED. **Founder visual review needed** — camera UI changed substantially. Open minor: permission-denied screen lacks Back affordance. |
| 10   | `f42db55`             | Photo upload mutations — `convex/consultations/photos.ts` exports `generatePhotoUploadUrl({token})` + `recordPhoto({token, consultationId, slot, fileId, mimeType, fileSizeBytes})`. Session-token auth (NEVER `ctx.auth.getUserIdentity` — providers:[]). Ownership check on consultation. Soft-delete prior `(consultationId, slot)` rows on re-record. 15 MB size guard + invalid-size guard. Imports `mimeTypeValidator` from `./schema`, `photoSlotValidator` from `../lib/photoSlot`. Tests at `convex/__tests__/consultations/photos.test.ts` (5 tests: invalid-slot, valid-insert, overwrite-soft-delete, ownership-rejection, file_too_large).     |

| 11 | `9f7a73d` | Google Sign-In — `convex/auth/socialSignIn.ts` Node action `signInWithGoogle({idToken})` verifies via `OAuth2Client` against `GOOGLE_OAUTH_CLIENT_ID`. `convex/auth/socialDb.ts` `findOrCreateUserByEmail` mutation (V8 runtime). Mobile `GoogleSignInButton` uses `expo-auth-session/providers/google` `useIdTokenAuthRequest`. Wired into `welcome.tsx` above phone CTA. `google-auth-library@10.6.2` + `expo-auth-session@55.0.15` exact-pinned. `apps/mobile/.env.example` documents env var placeholders. 4 vitest cases (invalid token, new user, repeat returns same userId, missing env). |
| 12 | `0bb87e0` | Apple Sign-In — `signInWithApple({identityToken, nonce?})` action verifies via `jose` `jwtVerify` against `APPLE_OAUTH_CLIENT_ID`. Mobile `AppleSignInButton` returns null on non-iOS; uses `expo-apple-authentication`. Wired below GoogleSignInButton. `jose@6.2.2` exact-pinned. 4 vitest cases (valid, missing env, no email, nonce mismatch). NOTE: client-side nonce generation NOT wired — server validates but client doesn't supply. Logged in DEFERRED.md as Phase 8 polish; TODO comment at AppleSignInButton.tsx:20. |
| 13 | `4f8caf0` | `startConsultation` + `submitConsultation` + `aiStub` — `convex/consultations/submitConsultation.ts` two mutations: startConsultation creates SUBMITTED row owned by caller; submitConsultation guards 4 distinct photos + idempotency (no prior questionnaire_responses) + ownership, writes responses row, schedules `internal.consultations.aiStub.kickoffAiStub`. `convex/consultations/aiStub.ts` Phase-3B placeholder transitions SUBMITTED → AI_PROCESSING (Phase 3C replaces with real Claude call; contract stays). Hook `apps/mobile/src/hooks/use-submit-consultation.ts` exposes `{submit(consultationId), submitting, error}`. 5 vitest cases (missing photos, happy path with `vi.useFakeTimers()` to flush scheduler, duplicate, ownership, startConsultation creates SUBMITTED). |
| 14 | `f847e6d` | Wire review Submit + photo capture to Convex — `questionnaire-store.ts` adds `consultationId: Id<"consultations"> \| null` + `setConsultationId`. `[condition]/index.tsx` flow entry calls `startConsultation({vertical:"hair_loss"})` after `startHL` and persists id. `review.tsx` Submit handler uses `useSubmitConsultation` (replaces placeholder). `apps/mobile/src/questionnaire/uploadPhoto.ts` shared helper: `generatePhotoUploadUrl` → POST upload → `recordPhoto`. `camera.tsx` confirm: upload first then setPhotoUri + back; "Use this photo" disabled while uploading. `pickFromLibrary.ts` accepts `{convex, token, consultationId}` deps. `jest.setup.ts` global `useConvex` mock. 3 existing test files surgically updated. 260/260 stays. |
| 15 | `7c9a922` | Phase 3B post-review fixes — I-2 (double-tap guard on Start assessment via `starting` state + disabled button) + I-5 (Alert.alert on library-pick upload failure) + I-3 TODO comment in AppleSignInButton.tsx for nonce wiring deferral. Phase code-review APPROVED_WITH_MINORS at `docs/superpowers/reviews/2026-04-25-phase-3b-review.md`. |

**Tests at session end:** convex **250/250 vitest** · 1 skipped · mobile **260/260 jest** (net +23 from 237 baseline) · workspace typecheck clean (6/6) · workspace lint clean (4 pre-existing import/order warnings in @onlyou/ui).

**Phase code-review verdict:** APPROVED_WITH_MINORS. 2 Important findings fixed before merge (I-2, I-5); 4 Important + 9 Minor deferred to Phase 8 with ledger entries (`docs/DEFERRED.md` "Phase 3B post-review deferrals (2026-04-25)"). Full report at `docs/superpowers/reviews/2026-04-25-phase-3b-review.md`.

**DEFERRED strikes applied (2026-04-25):**

- Line 60 — Hair Loss questionnaire content → SHIPPED `5af8b37` + `787f5fd` + `c4fa297`
- Line 66 — Social auth (Google + Apple) → SHIPPED `9f7a73d` + `0bb87e0`
- Line 277 — Photo upload "Choose from library" → SHIPPED `60b173e` + `488955c` + `f847e6d`
- Line 278 — Real camera → SHIPPED `488955c` + live capture via `f847e6d`

**TaskList state:** Tasks 1–14 + 15 (review + DEFERRED + fix commit) completed. Task 15.1 (the actual merge) awaits founder approval after live E2E (Task 14.1).

**Awaiting founder action (BEFORE MERGE):**

1. **Live E2E on physical device** (Task 14.1 — plan §14 Step 4): sign up fresh → 28-Q questionnaire → upload 4 real photos → submit. Expected observable state in Convex dashboard:
   - consultation row with `status: "AI_PROCESSING"` (stub transitioned)
   - questionnaire_responses row with answers JSON
   - 4 photos rows (one per slot, all live, none soft-deleted)
   - consultation_status_history row: `SUBMITTED → AI_PROCESSING (kind: system)`
   - Record screenshots + consultationId at `docs/superpowers/phases/3b/e2e/`
2. **Founder visual review** of the new camera UI (substantial change from Phase 2C dark-screen mock — live `CameraView`, capture-then-preview state, permission-denied screen)
3. **Set Convex envs on dev deployment** before live E2E: `pnpm convex env set GOOGLE_OAUTH_CLIENT_ID=...` (server-side); set `EXPO_PUBLIC_GOOGLE_OAUTH_CLIENT_ID=...` in `apps/mobile/.env.local` (client-side). Apple OAuth client ID set similarly. Without these, social-auth buttons disable safely.
4. **Approve merge:** `cd /d/onlyou2 && git merge --no-ff phase-3b -m "Merge phase-3b: questionnaire + photos + consultations"`. Then `git worktree remove ../onlyou2-phase-3b && git branch -D phase-3b`.

**Phase-3b branch tip:** `7c9a922`, 16 commits ahead of master `59b16bf`. Master is also ~70 commits ahead of `origin/master` (carryover from 2.5E + 3A); push deferred to founder.

**Org-cap incident (2026-04-25):** one transient `You've hit your org's monthly usage limit` on a Task 6 subagent dispatch; retry same prompt succeeded.

**Open against origin:** master still ~70 commits ahead of `origin/master` (carryover from 2.5E + 3A). Phase 3B commits live only on local `phase-3b` branch.
