...
**Current phase:** Phase 2.5E — ✅ **COMPLETE on `phase-2.5e` branch (tip `b440c66`, will advance after T21 commit), awaiting live E2E + merge.** 21 commits across 5 waves. Real reference-range bounds + multi-point trend + prior-report `prev` now join in `myBiomarkerReports`. Hook transform consumes real server shape with direction-aware status, unbounded-range synthesis, and latest-report-only semantics. `RangeBar` gains `direction` prop (gradient stops adapt for unboundedHigh/Low). Dashboard greeting/date/avatar wired to `useDisplayUser` + IST helpers. Detail byline reads "— Your care team". Explainer module now `{base, byStatus}` with 31 entries covering mock ids + canonical ids. Dev seed `seedArjunHistory` generates multi-report drifting history for trend E2E. Env-verify doc committed. Code review APPROVED*WITH_MINORS (0 Critical, 3 Important, 5 Minor); I-2 (canonical-id mapping) + I-3 (ISO-8601 ingest normalization) fixed `43088c4` / `b440c66`; I-1 handled in this commit. **Tests:** mobile **236/236 jest** · convex **218/218 vitest** · seed **19/19 vitest** · `pnpm -w typecheck --force` clean *(counts carried from T20 code-review report; not re-run in this doc-only commit)\_. **Open:** T19 live E2E on physical Android (user-gated — seed history + toggle EXPO_PUBLIC_USE_MOCK_BIOMARKERS=0 + capture 4 screenshots). T22 merge (user approval gate). iOS parity still Mac-blocked (re-deferred to Phase 3). **Next action:** user runs live E2E, then grants merge approval.
