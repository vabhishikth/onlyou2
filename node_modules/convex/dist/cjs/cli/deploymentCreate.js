...
async function createLocalDeployment(ctx, currentDeployment, select) {
  ...
  const devCommand = select ? "npx convex dev" : "npx convex dev --deployment local";
  (0, import_log.logMessage)(`
...
async function resolveOptionsInteractively(ctx, currentDeployment, refParam, options) {
  ...
  while (ref === void 0) {
    const input = await (0, import_prompts.promptString)(ctx, {
      message: "How to name this deployment?\n" + import_chalk.chalkStderr.reset.dim(
...
function parseSelectorForNewDeployment(selectorString) {
  ...
  switch (selector.kind) {
    case "local":
      return {
        kind: "invalid",
        message: `"local" is not a valid deployment reference. To create a local deployment, run ${import_chalk.chalkStderr.bold("npx convex deployment create local")}`
      };
    case "deploymentName":
      return {
        kind: "invalid",
...
function validateTentativeReference(tentativeReference) {
  ...
  if (tentativeReference === "local") {
    return `"local" is not a valid deployment reference. To create a local deployment, run ${import_chalk.chalkStderr.bold("npx convex deployment create local")}`;
  }
