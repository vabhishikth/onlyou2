...
### What the Patient Gets for Free (Before Payment)
...
3. **Personalized treatment plan** — doctor creates a plan with specific medications and regimen

### What's Included in Every Subscription (After Payment)
1. **E-prescription** — generated from condition-specific templates, PDF stored
2. **Medication** — discreet local delivery with OTP confirmation
3. **Ongoing check-ins** — 4-week, 3-month, 6-month cadence
4. **Unlimited messaging with your doctor** — ask questions anytime during active subscription
5. **First blood panel** — included when clinically indicated (not all verticals need it)
6. **Auto-reorder** — medication automatically reordered on subscription renewal

### Why Free Consultation?
The initial consultation is the **customer acquisition cost**, not a revenue line. Revenue comes from treatment subscriptions. This model is proven by:
- **Man Matters / Be Bodywise** (India) — free consultations, 1M+ customers
- **Ro/Roman** (US) — free consultations for most services, scaled to profitability
- **MFine** (cautionary tale) — tried monetizing consultations, lost ₹102 Cr on ₹12.9 Cr revenue, nearly collapsed

For stigmatized conditions specifically, a free doctor interaction eliminates the two biggest barriers: "Is this worth my money?" and "Will I be judged?"
...
AI PRE-ASSESSMENT (Claude analyzes → classification, risk flags, contraindications)
        │
        ▼
DOCTOR REVIEWS CASE — FREE (specialist sees AI summary + raw data → 3-8 min per case)
        │
        ├─── CREATES TREATMENT PLAN ─────────────────────┐
        │    (prescription + regimen ready)               │
        │                                                 ▼
        │                                    PATIENT SEES TREATMENT PLAN
        │                                    "Dr. [Name] has reviewed your case
        │                                     and created a personalized plan."
        │                                                 │
        │                                                 ▼
        │                                    SELECTS PLAN & PAYS
...
### Key Flow Difference: Payment Comes AFTER Doctor Review

**Old model (rejected):** Questionnaire → Photos → Payment → Doctor reviews
**New model (implemented):** Questionnaire → Photos → Doctor reviews (FREE) → Treatment plan presented → Payment

**Why:** For stigmatized conditions in India, patients won't pay without first knowing a doctor has reviewed their case and can help them. The consultation is the acquisition cost; revenue comes from treatment subscriptions.

**Anti-abuse safeguard:** One free consultation per vertical per user. Returning patients who abandoned after treatment plan must subscribe to get another consultation.
