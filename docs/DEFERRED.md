...
| **`apps/mobile/tsconfig.json` had `"ignoreDeprecations": "6.0"`** — only `"5.0"` is a valid value for the current TS version. Pre-existing since Phase 1 (`9a3a1fa`); IDE TS server flagged it this session. Fixed on master + worktree.                                                                                                                                                 | ✅ Fixed                              | —                                               | One-line fix shipped on master and the 2C branch. Slipped past `pnpm typecheck` because that runs a different TS path than the IDE server.                                                      |
| **Convex agent skill files keep showing as dirty** — `.agents/skills/convex-*/**`, `.claude/skills/convex-*/**`, `CLAUDE.md`, `skills-lock.json`, `apps/mobile/expo-env.d.ts`, `convex/_generated/ai/` are auto-rewritten every time `npx convex dev` boots. They were committed historically, so they show as modified on every dev session.                                            | ⏳ Pending                            | Phase 2C walkthrough patch                      | Add to `.gitignore` and remove the tracked copies. Convex regenerates them from its CLI on demand — they should never have been versioned. Avoids per-session noise in `git status`.            |
