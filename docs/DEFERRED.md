...
| Item                                                                                                                                                       | Deferred to                      | Why                                                                                                                                                                                                            |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `biomarker_reference_ranges` Convex table + schema (with `aliases` field + age/sex-aware lookup + `source` citation)                                       | Phase 2.5 (Biomarker foundation) | Prerequisite for real classification — must precede real parsing. **Schema is extensible — adding a new marker is an insert, not a code change.**                                                              |
| Seed data for ~25 common markers — **MVP starting inventory, not a ceiling**                                                                               | Phase 2.5 (Biomarker foundation) | Medical review + source citations + seeder. DB grows over time as new markers are curated.                                                                                                                     |
...
| **Adaptive `parseLabReport` Convex action** (marker-agnostic — extracts every marker in the report, doesn't require DB match)                              | Phase 2.5 (Biomarker foundation) | **Core principle: biomarker output is driven by what the report contains, not a pre-chosen list. Two reports for the same patient can look completely different.**                                             |
...
| `biomarker_curation_queue` table (captures every unclassified marker extraction to drive the curator's backlog)                                            | Phase 2.5 (Biomarker foundation) | Unknown markers surfaced in the product become the clinical team's next curation batch                                                                                                                         |
| **Adaptive narrative generation** (Claude-generated "In summary" references only the markers in _this_ report — no template strings keyed to a fixed list) | Phase 2.5 (Biomarker foundation) | Narrative must adapt to variable report contents                                                                                                                                                               |
...
Items surfaced during the Phase 2C code review (`docs/superpowers/reviews/2026-04-14-phase-2c-tab-content-consultation-review.md`). All six BLOCKER/MAJOR/MINOR fixes landed in the fix pass; these four items are intentional carry-forwards that need real Convex wiring or fall outside Phase 2C scope.
