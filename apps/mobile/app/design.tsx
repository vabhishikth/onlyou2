...
export default function DesignScreen() {
  return (
    <ScreenWrapper>
      <Logo size={48} />
      <Text style={{ fontSize: 11, color: colors.textTertiary, textTransform: 'uppercase', letterSpacing: 2, marginTop: 8, marginBottom: 32 }}>
        Design system — @onlyou/mobile
      </Text>

      <Section title="Typography">
        <Text style={textStyles.h1}>Your care, private</Text>
        <Text style={[textStyles.h2, { marginTop: 12 }]}>Treatment that fits you</Text>
        <Text style={[textStyles.h3, { marginTop: 12 }]}>Book a free consultation</Text>
        ...
        <Text style={[textStyles.bodySecondary, { color: colors.textSecondary, marginTop: 8 }]}>
          Takes about 5 minutes. Your answers stay private.
        </Text>
        <Text style={[textStyles.small, { color: colors.textTertiary, marginTop: 8 }]}>
      ...
      <Section title="PremiumInput">
        <View style={{ gap: 16 }}>
          ...
          <PremiumInput label="Email" placeholder="you@example.in" error="This email is not valid" />
        </View>
      </Section>

      <Section title="Status Badges">
        <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          <Chip label="COMPLETED" color={colors.success} bg={colors.successBg} />
          <Chip label="AWAITING REVIEW" color={colors.warning} bg={colors.warningBg} />
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <View style={{ marginTop: 32, paddingTop: 24, borderTopWidth: 1, borderTopColor: colors.border }}>
      <Text style={{ fontFamily: 'PlayfairDisplay_700Bold', fontSize: 24, marginBottom: 16 }}>{title}</Text>
      {children}
    </View>
  )
}
...
function Swatch({ name, hex }: { name: string; hex: string }) {
  return (
    <View style={{ marginRight: 12, width: 100 }}>
      <View style={{ height: 80, borderRadius: 12, backgroundColor: hex, borderWidth: 1, borderColor: colors.border }} />
      <Text style={{ fontSize: 12, fontFamily: 'PlusJakartaSans_600SemiBold', marginTop: 8 }}>{name}</Text>
      <Text style={{ fontSize: 10, color: colors.textTertiary, marginTop: 2 }}>{hex}</Text>
    </View>
  )
}
...
function Chip({ label, color, bg }: { label: string; color: string; bg: string }) {
  return (
    <View style={{ backgroundColor: bg, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 999 }}>
      <Text style={{ color, fontSize: 11, fontFamily: 'PlusJakartaSans_700Bold' }}>{label}</Text>
    </View>
  )
}
